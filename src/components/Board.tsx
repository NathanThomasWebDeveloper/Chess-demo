...
const Board: board = ({piecesToRender, playingColor = "WHITE"}) => {
    ...
    const pieceElements = squareSize && top && left && piecesToRenderWithEmitters && piecesToRenderWithEmitters.map(({...props}) =>
        <Piece key={props.name + props.position[0] + props.position[1]} {...props} topOffset={top} leftOffset={left}
               squareSize={squareSize}/>)
    return (<>
            <BoardContext.Provider value={playingColor}>
                <div
